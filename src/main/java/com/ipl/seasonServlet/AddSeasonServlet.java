package com.ipl.seasonServlet;

import javax.servlet.ServletException;
import javax.servlet.annotation.WebServlet;
import javax.servlet.http.HttpServlet;
import javax.servlet.http.HttpServletRequest;
import javax.servlet.http.HttpServletResponse;
import java.io.IOException;
import java.io.PrintWriter;

import com.ipl.serviceIpl.SeasonServiceImpl;
import com.ipl.serviceIpl.InterSeasonService;

public class AddSeasonServlet extends HttpServlet {
      InterSeasonService service = new SeasonServiceImpl();

    protected void doPost(HttpServletRequest request, HttpServletResponse response)
            throws ServletException, IOException {

        response.setContentType("text/html");
        PrintWriter out = response.getWriter();

        try {
            // Get the season parameter from the form
            String seasonParam = request.getParameter("season");
            if (seasonParam == null || seasonParam.trim().isEmpty()) {
            	out.println("<h3>Season can not be null and empty!!!!</h3>");
		}

            int season = Integer.parseInt(seasonParam);
            
                   int rows =  service.addSeason(season);
              if(rows > 0){            

            out.println("<h3>Season " + season + " added successfully!</h3>");
            out.println("<a href='index.html'>Back to Main Menu</a>");
              } else{
              
               out.println("<h3>Season " + season + " Not added !!!</h3>");
            out.println("<a href='index.html'>Back to Main Menu</a>");
              
              }
        } catch (NumberFormatException e) {
            out.println("<h3>Error: Season must be a valid number.</h3>");
            out.println("<a href='index.html'>Back to Main Menu</a>");
        }
         finally {
            out.close();
       		 }
	}    
}

